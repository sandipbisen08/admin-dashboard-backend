...
exports.list = async (req, res) => {
  try {
    const items = await AboutDetail.find().sort({ createdAt: -1 });
    res.json(
      items.map((d) => {
        return {
          id: d._id,
...
exports.update = async (req, res) => {
  try {
    ...
    const existing = await AboutDetail.findById(req.params.id);
    if (!existing) {
    ...
    if (req.file) {
      updateDoc.imagePath = `/uploads/about/${req.file.filename}`;
    }

    const updated = await AboutDetail.findByIdAndUpdate(req.params.id, updateDoc, {
      new: true,
      runValidators: true
    });
...
exports.remove = async (req, res) => {
  try {
    const deleted = await AboutDetail.findByIdAndDelete(req.params.id);
    ...
    res.json({ message: 'About detail deleted' });
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
  }
